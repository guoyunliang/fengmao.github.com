...
<ul>
<li>读取配置文件，建立保存group信息的数据结构，这些信息包括该group的数据服务器列表等;
</li>
<li>
如果主配置服务器存在并且alive,备配置服务器从主配置服务器上读取group的配置信息，而不是从配置文件中读取;
</li>
<li>
建立循环，循环做如下事情:
<ul>
<li>检查group的配置文件版本信息，确定是否要重新读取配置文件；</li>
<li>检查group的对照表，确定是否要重新建立对照表，若需重建，设置重建标志；</li>
<li>检查配置服务器的状态；</li>
<li>检查数group的数据服务器的状态；</li>
</li>
</ul>
