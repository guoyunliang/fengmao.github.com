...
在TairManager::update_server_table函数的结尾处，调用MigrateManager::set_migrate_server_list函数，我们看看该函数干来什么

<pre class="prettyprint lang-cc">
ger::set_migrate_server_list(bucket_server_map migrate_server_list, uint32_t version)
{
  //为什么要先睡一会儿? 这个地方我还没有完全明白
  TAIR_SLEEP(_stop, 1);
  tbsys::CThreadGuard guard(&get_data_mutex);
  //将需要迁移的bucket以及目标server_id存储在migrate_servers中，并通知migrate线程，开始干活了。
...
接下来看看DS的迁移线程在作什么吧。
MigrateManager::Run() => MigrateManager::Do_Run() => MigrateManager::Do_Migrate()
<pre class="prettyprint lang-cc">
void migrate_manager::do_migrate()
{
  //<bucket_id, {dest_server_1, dest_server_2,...}>
...
数据迁移是Tair的CS与DS相互协同，完成数据迁移的。
<ul>
<li>1)Tair CS周期性的扫描group的信息，首先发现group状态发生变化，需要重建对照表，将该group丢给创建对照表线程；</li>
<li>2)CS的创建对照表线程按照指定的策略，创建来对照表，增加对照表版本号，并检查是否需要数据迁移，并记录需要迁移的bucket,将对照表通过心跳发送到数据服务器；</li>
<li>3)Tair DS收到心跳包后，开始更新对照表，检查需要迁移的bucket; 由一个线程开始迁移bucket, 没迁移完成一个bucket, 就向CS会报；</li>
<li>4)Tair CS收到DS发送过来的Bucket迁移完成的回包后，将修改m_hash_table, 以反映最新的bucket的分布情况；然后增加对照被的版本号；</li>
<li>5)Tair DS重复步骤3）直到m_hash_table和d_hash_table内容完全一样；</li>
<li>6)Tair CS重复步骤4）直到数据迁移完成；</li>
</ul>
这是我们期望的过程，实际上数据迁移要比这个复杂；
例如，在迁移过程中，强制重建对照表，会发生什么？ 迁移过程中有目标DS失效，或者源DS失效，会如何处理？ 这两个问题很重要，留在后面回复。
上述过程，可以通过下图来描述：
====================================
